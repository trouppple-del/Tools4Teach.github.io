...
function menuSelectTab(tab) {
    ...
    // Update toolbar active state - toolbar items: 0=coffee, 1=seating, 2=wheel
...
window.switchTab = function(tab, button) {
    // Update toolbar buttons active state - toolbar items: 0=coffee, 1=seating, 2=wheel
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Mark seating plan button as active by default (index 1)
    const toolbarItems = document.querySelectorAll('.toolbar-item:not(.export-seating-only)');
    ...
    if (removeFromWheelBtn) removeFromWheelBtn.addEventListener('click', () => {
        if (groupsState.selectedStudent) {
            ...
            if (index !== -1) {
                ...
                drawWheel();
                // Hide the result if no students left
...
function loadClasses() {
    try {
        ...
        // Load global constraints from seatingAppData OR from classes
        try {
            ...
            if (globalData) {
                ...
                // Fallback: collect constraints from all classes
...
function updateStudentsList() {
    ...
    if (groupsState.students.length === 0) {
        ...
        // Update constraints list if element exists
...
// Update constraints list display for current class
function updateConstraintsList() {
    ...
    // Filter constraints for current class students
...
function generateGroups() {
    ...
    if (groupsState.applyConstraintsToGroups && groupsState.constraints && groupsState.constraints.length > 0) {
        ...
        // Place together groups first (just the together group members, not filled yet)
        const placed = new Set();
        const groups = [];

        // Add together groups as-is (don't fill them yet)
        ...
        // Add remaining students, respecting separate constraints
        ...
        // Create a map of students who cannot be together
        ...
        // Create groups from remaining students, distributing evenly
        let i = 0;
        while (i < shuffled.length) {
            // Find the smallest existing group to add to, or create new group
            let targetGroup = null;
            let smallestSize = groupSize;
            
            for (const group of groups) {
                if (group.length < groupSize) {
                    // Check if we can add to this group (no separate constraints violated)
                    let canAdd = true;
            ...
            if (targetGroup) {
                ...
                while (newGroup.length < groupSize && i < shuffled.length) {
                    const student = shuffled[i];
                    // Check if student can be added to this new group
                    let canAdd = true;
                    const conflicts = cannotBeTogether.get(student.id);
                    if (conflicts) {
                    ...
                    if (canAdd) {
                        ...
                        // Skip this student for now, try next one
                        i++;
                ...
                if (newGroup.length > 0) {
                    ...
                    break; // No more students can be placed
                }
            }
        }
