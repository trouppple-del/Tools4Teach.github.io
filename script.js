...
function addConstraint(student1, student2) {
    ...
    if (!existing) {
        state.constraints.push({ student1, student2, type: state.constraintType });
        saveState();
        renderConstraintsList();
    }
}
...
function showConstraintModal() {
    ...
    modal.innerHTML = `
        ...
        <div style="display: flex; gap: 10px; margin-bottom: 15px; flex-direction: column;">
            ...
            <select id="constraintSelect1" style="padding: 8px; background-color: var(--surface-light); border: 1px solid var(--border); border-radius: 6px; color: var(--text);"></select>
            <label style="color: var(--text-muted); font-size: 12px; margin-top: 10px;">Student 2</label>
            <select id="constraintSelect2" style="padding: 8px; background-color: var(--surface-light); border: 1px solid var(--border); border-radius: 6px; color: var(--text);"></select>
        </div>
        <div style="display: flex; gap: 10px;">
            <button onclick="this.parentElement.parentElement.remove()" style="flex: 1; background-color: var(--secondary);">Cancel</button>
            <button onclick="
                const s1 = parseInt(document.getElementById('constraintSelect1').value);
                const s2 = parseInt(document.getElementById('constraintSelect2').value);
                addConstraint(s1, s2);
                this.parentElement.parentElement.remove();
            " style="flex: 1; background-color: var(--success);">Add</button>
        </div>
    `;
    
    document.body.appendChild(modal);
    
    const select1 = modal.querySelector('#constraintSelect1');
    const select2 = modal.querySelector('#constraintSelect2');
    ...
    if (state.students.length > 1) {
        select2.value = state.students[1].id;
    }
}
...
function generateSeatingPlan() {
    ...
    for (const group of togetherGroups) {
        ...
        if (members.some(s =>
            state.tables.some(t => t.students.some(ts => ts.id === s.id))
        )) continue;

        let placed = false;

        for (const table of state.tables) {
            if (table.students.length + members.length > table.capacity) continue;

            let valid = true;

            for (const m of members) {
                if (state.constraints.some(c =>
                    c.type === 'separate' &&
                    (
                        (c.student1 === m.id && table.students.some(ts => ts.id === c.student2)) ||
                        (c.student2 === m.id && table.students.some(ts => ts.id === c.student1))
                    )
                )) {
                    valid = false;
                    break;
            ...
            for (const m of members) {
                table.students.push({ ...m, locked: false });
                assigned.add(m.id);
            }
    ...
    for (const student of unassigned) {
        ...
        // First, try to place with together constraints
        const group = togetherGroups.find(g => g.includes(student.id));
        if (group) {
            const others = group.filter(id => id !== student.id);
            const seated = others.filter(id =>
                state.tables.some(t => t.students.some(s => s.id === id))
            );

            if (seated.length > 0) {
                // Some group members are seated, try to place at their table
                for (const seatedId of seated) {
                    for (const table of state.tables) {
                        if (table.students.some(s => s.id === seatedId)) {
                            // Found table with seated member
                            if (table.students.length < table.capacity &&
                                !state.constraints.some(c =>
                                    c.type === 'separate' &&
                                    (
        ...
        // If not placed yet, try any valid table
        if (!placed) {
            for (const table of state.tables) {
    ...
    // Second pass: assign any remaining unassigned students
    const stillUnassigned = state.students.filter(s =>
        !state.tables.some(t => t.students.some(ts => ts.id === s.id))
    );

    for (const student of stillUnassigned) {
        // Find the table with the most space available
        let bestTable = null;
        let maxSpace = -1;

        for (const table of state.tables) {
            const space = Math.max(0, table.capacity - table.students.length);
            if (space > maxSpace && !state.constraints.some(c =>
                c.type === 'separate' &&
                (
                    (c.student1 === student.id && table.students.some(ts => ts.id === c.student2)) ||
                    (c.student2 === student.id && table.students.some(ts => ts.id === c.student1))
        ...
        if (bestTable && maxSpace > 0) {
            bestTable.students.push({ ...student, locked: false });
        }
    }
...
if (bulkAddBtn) {
    bulkAddBtn.addEventListener('click', () => {
        ...
        modal.querySelector('#bulkAddConfirm').addEventListener('click', async () => {
            const textNames = textarea.value.split(/[\s,]+/).filter(n => n.trim());
            ...
            if (fileInput.files[0]) {
                ...
                lines.forEach(line => {
                    ...
                    if (name && name.length > 0 && !name.includes('?')) addStudent(name);
                });
            }
